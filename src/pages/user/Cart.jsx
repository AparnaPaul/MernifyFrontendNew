...
const Cart = () => {
  const { cart, totalItem, subTotal, clearCart, updateCart, removeFromCart } = CartData()
      ...
      <h1 className='text-3xl font-bold mb-6 text-center'>Your Cart</h1>{
        cart.length === 0 ? <div className='text-center py-10'>
          ...
          <div className='lg:col-span-2 space-y-6'>
            {
              cart.map((e) => (
                <div key={e._id} className='flex flex-col sm:flex-row items-center sm:items-stretch space-y-4 sm:space-y-0 sm:space-x-4 shadow-md rounded-lg p-4 border border-gray-400'>
                  ...
                  />
                  <div className='flex-1 text-center sm:text-left'><h2 className='text-lg font-medium'>{e.product.title}</h2>
                    <p>Price: $ {e.product.price}</p>
                    <Button variant="outline" size="sm" onClick={() => updateCartHandler("decrement", e._id)}>-</Button>
                    <span className='text-center p-2'>{e.quantity}</span>
                    <Button variant="outline" size="sm" onClick={() => updateCartHandler("increment", e._id)}>+</Button>
                  </div>
          ...
          <div className='p-6 shadow-lg rounded-lg border border-gray-400'>
            <h1 className='text-xl font-semibold mb-4 text-center lg:text-left'>Order Summary</h1>
            ...
            <div className='flex justify-between font-medium text-lg'></div>
            <span className='text-l font-semibold'>Total : </span>
